...
class Redis(_Redis):
    """
    Redis 继承类，由于Ｄjango默认只对redis做了简单的封装，redis很多原生的指令无法得到使用，所以我们需要
    自定义封装一套接口

    接口与原生Ｒedis 保持一致，增加自动序列化，饭序列化功能
    ...
    def keys(self,pattern='*'):
        # 原生的keys是取出redis所有的杂乱无章的key，我们进行优化排序使其变得更加整洁
        'Returns a list of keys matching ``pattern``'
        return sorted(_Redis.keys(self,pattern))

    def set(self,key,value,timeout=0):
        #setex 设置值的时候，同时设置过期时间
        if timeout > 0:
...
class MSRedis(object):
    ...
    def __init__(self,conf):
        ...
        self.read_commands = [
            ...
            'hget','hgetall','hkeys','hmget','sismember',
            'smember','sdiff','sinter','sunion',
